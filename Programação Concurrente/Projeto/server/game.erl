-module(game).
-export([game/3,sendDead/2,colisionMapBorder/1,colisionSun/1,colisionDetectionPlayer/2,colisionDetection/4, updatePlayer/2,updateMob/2, processPlayer/4,initPlayerPos/3, addPlayer/13, printList/1, sendMob/3,listTuple_to_Command/3, tuple_to_string/1, initMobs/0, generateMobs/2]).


%Game function
game(PlayersPID, PlayerList, MobList) ->
    receive
        %init game Data
        {start, Players} -> 
            NewPlayerList=initPlayerPos(Players,1,[]),
            NewMobList=initMobs(),
            %printList(NewPlayerList),
            %printList(NewMobList),
            sendMob(Players,"player" ,NewPlayerList),
            sendMob(Players,"mob" ,NewMobList),
            timer:sleep(1),
            game(Players,NewPlayerList,NewMobList);
        {updateCliente, Command, ID} ->
            ...
            timer:sleep(1),
            game(PlayersPID,NewPlayerList,MobList)
    after 1 -> 
        NewPlayerList = colisionDetection(PlayerList,MobList,[],PlayersPID),
        NewPlayerList2 = updatePlayer(NewPlayerList,[]),
        NewMobList = updateMob(MobList,[]),
        sendMob(PlayersPID,"player" ,NewPlayerList2),
        sendMob(PlayersPID,"mob" ,MobList),
        game(PlayersPID,NewPlayerList2,NewMobList)
    end.
                ...
                "up" -> 
                    ...
                    case F of
                        _ when F < 0.1 ->
                            ...
                            Fuel = 0.0;
                        _ ->
                            VeloX = list_to_float(element(5,H)) + 0.02*CosValue,
                            VeloY = list_to_float(element(6,H)) + 0.02*SinValue,
                ...
                "left" -> 
                    ...
                    case F of
                        _ when F < 0.1 ->
                            ...
                            Fuel = 0.0;
                        _ ->
                            Rotation = list_to_float(element(12,H))-0.01,
...
initPlayerPos([H|T],N,PlayerList)->
    case N of
        ...
        4 -> 
            NewPlayerList=addPlayer(PlayerList, "4","1200.0", "650.0", "50.0", "0.0","0.0" ,"120.0", "250","0", "250", "100.0","0.0")
    end,
...
colisionDetection([H|T],MobList, NewPlayerList, PlayersPID) ->
    case colisionDetectionPlayer(H,MobList) of
        dead ->
            %enviar uma mensagem para o player
            sendDead(PlayersPID,element(1,H)),
...
%Just for debugging and know what is inside the list
printList(MobList) ->
    FormattedList = lists:flatten(io_lib:format("~p", [MobList])),
    io:format("~s~n", [FormattedList]).
